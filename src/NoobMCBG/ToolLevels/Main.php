...
use pocketmine\network\mcpe\protocol\{PlaySoundPacket, LevelSoundEventPacket, LevelEventPacket};
...
class Main extends PB implements L {
    ...
    $level = $player->getLevel();
            if(!isset($this->cooldown[$player->getName()])){
                $this->cooldown[$player->getName()] = time() + 3;
    ...
    public function MenuSkillToolLevel($player){
        $form = new SimpleForm(function(Player $player, $data){
            ...
            switch($data){
                ...
                break;
                case 1:
                    if(!$player->hasPermisson("toollevel.pickaxeleveling")){
